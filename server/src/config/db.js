...
async function autoSeedMemoryDB() {
...
export async function connectDB(uri = config.mongodbUri) {
  try {
    ...
    if (!config.isProduction) {
      console.log(`[MongoDB] Local MongoDB unreachable. Starting built-in in-memory database...`);
      const { MongoMemoryServer } = await import('mongodb-memory-server');
      mongoMemoryInstance = await MongoMemoryServer.create();
      const memUri = mongoMemoryInstance.getUri();
      const conn = await mongoose.connect(memUri);
      console.log(`[MongoDB] In-memory database running.`);
      await autoSeedMemoryDB();
      return conn;
    }
